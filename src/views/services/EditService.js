...
function EditService() {
  ...
  const fetchDataById = async (setValues) => {
    try {
      const response = await axiosClient.get(`admin/service/${id}/edit`)
      const data = response.data.service
      if (data) {
        setValues({
          title: data?.service_desc?.title,
          friendlyUrl: data?.service_desc?.friendly_url,
          pageTitle: data?.service_desc?.friendly_title,
          metaKeyword: data?.service_desc?.metakey,
          metaDesc: data?.service_desc?.metadesc,
          visible: data?.display,
        })
        setEditorData(data?.service_desc?.description)
      ...
      console.error('Fetch data id service is error', error.message)
    }
  }
